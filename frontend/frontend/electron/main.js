...
function createWindow() {
  ...
  if (isDev) {
    ...
    // В Electron приложении dist находится в app.asar или в resources
    let frontendPath;
    ...
    frontendPath = possiblePaths.find(p => fs.existsSync(p));
    
    if (!frontendPath) {
      console.error('Frontend index.html not found. Tried paths:');
      ...
      // Показываем ошибку пользователю
      mainWindow.webContents.send('frontend-error', 'Frontend files not found');
      return;
    }
